...
```
- `tools` is an allowlist. Values: bare name (enabled), `name: ask` (human approval per call), omitted (disabled). Default when `tools` is absent: `files` only. Groups: `read` (Read, Glob, Grep), `files` (read plus Write, Edit), `bash`, `web_fetch` (WebFetch — fetched from the run's own sandbox; `fetch` is the old spelling), `web` (WebFetch plus Anthropic's server-side WebSearch, the one built-in that runs off your machine and bills per search). An install that runs its own search engine sets `FOLDRUN_SEARCH_URL` and grants `web_fetch` with the account's `web_search` tool instead — then nothing but tokens ever leaves the box.
...
- `preview: <globs>` — what a gated step's approval box shows: comma-separated paths under `storage/`, globs allowed (`draft/*.mdx, draft/images/*.webp`), resolved when the run parks. Without it the box shows the previous group's replies and whatever files under `storage/` they name.
...
- `schema: …` — beside `output: json`, the JSON Schema the returned value must satisfy: one-line JSON (`schema: {"type": "array", "items": {"type": "string"}}`), a YAML block on the lines indented under the key, or a file under the workspace (`schema: ../../schemas/lead.json`, JSON or YAML). Checked where the value is parsed, on both executors, with the hand-written subset of JSON Schema — types, `required`, `properties`, `enum`, ranges, `pattern`, `items`, `additionalProperties`, `anyOf`/`oneOf`/`allOf`/`not`, local `$ref`; formats are accepted and not checked. A value that does not fit fails the step with the field named (`schema: the value does not fit — /url: does not match ^https://`), the same failure as no value at all. The schema is quoted whole in the step's prompt, so the model is told what it must produce rather than left to guess it from field names.
...
- `parallel: <n>` — on an `each:` step, how many instances run at once (1 to 20). Unset is all of them, which is what fan-out always did; the cap is for twenty browser steps against one site, or one vendor's rate limit. An instance waiting for a slot rents nothing, and the fan-out event says `6 instances, 2 at a time (parallel:)`.
- `max_turns: <n>` — the most model turns (a reply and its tool calls is one) this step may take before it is stopped, 1 to 500. The third bound beside `budget:` (money) and `timeout:` (time): a step that has not finished at the cap fails with `stopped after N turns (max_turns: in the flow file)`, keeping what it wrote and what it spent on the record. Unset, the runtime's own default stands.
