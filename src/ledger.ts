...
export interface RunMeter {
  ...
  steps: number;
  computeSecs: number;
  /** Seconds of computeSecs held at the small reservation; kept on the
   *  ledger line for the audit trail. Absent means all large. */
  smallSecs?: number;
  /** The hold+work meter: reserved core-seconds and GiB-seconds, burned
   *  CPU seconds, and the plain seconds of steps recorded before
   *  reservations were facts (priced by the flat fallback). */
  compute?: { coreSecs: number; gibSecs: number; busyCpuSecs: number; flatSecs: number };
 ...
 *   FOLDRUN_COMPUTE_USD_PER_CORE_SEC   the HOLD: each reserved core, per second
 *   FOLDRUN_COMPUTE_USD_PER_GIB_SEC    the HOLD: each reserved GiB, per second
 *   FOLDRUN_CPU_USD_PER_BUSY_SEC       the WORK: each CPU-second actually burned
 *   FOLDRUN_COMPUTE_USD_PER_SEC        flat per-second fallback for steps
 *                                      recorded before reservations were facts
 *
 * Hold + work is the century-old fair formula (utilities call it demand +
 * energy): the hold covers the slice occupied whether used or not — the
 * platform's real cost — and the work charge makes the busy run pay more
 * than the idle one. Per-core hold pricing also makes sizes price
 * themselves: a 1-core small pod pays a third of a 3-core large pod's hold
 * with no second rate knob. The work meter is the sandbox's own cgroup and
 * is best-effort — where it was unreadable the run pays hold only, because
 * a bill must never contain a guess.
...
interface PriceConfig {
  ...
  usdPerCoreSec: number;
  usdPerGibSec: number;
  usdPerBusyCpuSec: number;
  netUsdPerGb: number;
  minFee: number;
}
...
function priceConfig(): PriceConfig {
  return {
    ...
    usdPerCoreSec: envNum("FOLDRUN_COMPUTE_USD_PER_CORE_SEC", 0),
    usdPerGibSec: envNum("FOLDRUN_COMPUTE_USD_PER_GIB_SEC", 0),
    usdPerBusyCpuSec: envNum("FOLDRUN_CPU_USD_PER_BUSY_SEC", 0),
...
export function priceRun(meter: RunMeter | number): number {
  ...
  // at all — the industry's floor: a bill line reading "0.4s" invites a
  // dispute about measurement precision that a floor announces away. A run
  // with zero compute stays zero, because the floor never invents a bill.
  ...
  const c = priceConfig();
  // Hold + work. The floor rides the hold: a run that computed holds at
  // least one second of whatever it reserved. Steps without a recorded
  // reservation bill at the flat fallback, so old records stay priceable.
  const comp = m.compute;
  const computeCharge = comp
    ? Math.max(comp.coreSecs, comp.coreSecs > 0 ? 1 : 0) * c.usdPerCoreSec +
      comp.gibSecs * c.usdPerGibSec +
      comp.busyCpuSecs * c.usdPerBusyCpuSec +
      Math.ceil(comp.flatSecs) * c.computeUsdPerSec
    : secs * c.computeUsdPerSec;
  const charge = tokens * c.margin + c.runFee + computeCharge + gb * c.netUsdPerGb;
