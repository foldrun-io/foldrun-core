...
// grant is in the agent's own markdown (`tools: [name]`, or `scripts:` for
// a bare script), where a reviewer sees it.
//
...
export interface GalleryTool {
   ...
   * `tools: [name]`, with the args and timeout living in the definition
 ...
 * everywhere it appears — the frontmatter, and the `tools: [...]` line in
 * the body that would otherwise tell a reader to grant a tool that isn't
 * there.
 *
...
export function galleryTemplate(
  ...
  const rename = (text: string) =>
    text
      ...
      .replaceAll(`tools: [${tool.name}]`, `tools: [${name}]`);
